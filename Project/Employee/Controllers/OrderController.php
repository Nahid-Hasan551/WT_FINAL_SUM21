...
else if (isset($_POST["confrim_order"])){
  //do validations
  //if no error
  $rs = confrimOrder($_POST["id"]);
  if($rs === true){
...
function getAllOrder(){
  $query="select * from eorders";
  $rs=get($query);
  return $rs;
}

function getOrder($id){
  $query="select * from eorders where id=$id";
  $rs=get($query);
  return $rs[0];
}
...
function deleteOrder($id)
	{
		$query="delete from eorders  where id=$id";
		return execute($query);
	}
