...
else if (isset($_POST["delete_deliveryman"])){
  //do validations
  //if no error
  $rs = deleteDeliveryman($_POST["id"]);
  if($rs === true){
    header("Location:all_deliveryman.php");
  }
  $err_db = $rs;
...
function searchDeliveryman($key){
		$query = "select * from users where role='Delivery man' and username like '%$key%'";
		$rs = get($query);
		return $rs;
	}
 ?>